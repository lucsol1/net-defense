...
func _packet_phase() -> void:
	...
	packets_generated.emit(current_player(), packets)
	_main_phase()
...
func play_card(card_id: int) -> bool:
	...
	if not current_server().can_afford(cost):
		return false
	current_server().processing_power -= cost
	current_player().remove_card(card_id)
	return true

func activate_card(card_id: int) -> bool:
	var card = CardDatabase.get_card(card_id)
	if card.is_empty():
		return false
	var activate_cost = card.get("activate_cost", 0)
	if not current_server().can_afford(activate_cost):
		return false
	current_server().processing_power -= activate_cost
	var enemy_server = servers[1 - current_index]
	CardDatabase.apply_effect(card_id, current_player(), current_server(), enemy_server)
	return true
